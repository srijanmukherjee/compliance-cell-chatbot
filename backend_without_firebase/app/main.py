...
from app.routers import chat_model, intent, message_log, profile
...
origins = ["http://localhost:3001", "https://compliance-cell-chatbot-ha2g.vercel.app"]
...
@app.get("/")
async def index():
    return {"message": f"Hello from chai ☕ server!"}


@app.post("/chat")
async def chat(
    chat_message: ChatMessage, chat_service: ChatService = Depends(ChatService)
):
